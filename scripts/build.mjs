...
async function main() {
  ...
  function cachePath(...segments) {
    return path.join(cacheDir, upstream.owner, upstream.repo, upstream.ref, ...segments)
  }
  ...
  async function writeCachedText(filePath, text) {
  ...
  async function writeCachedJson(filePath, value) {
    await writeCachedText(filePath, `${JSON.stringify(value)}\n`)
  }

  async function fetchJsonCached(filePath, url) {
  ...
  async function fetchText(filePath, revision) {
  ...
  async function fetchTextCached(revision, filePath) {
  ...
  async function writeRefs(dir, revision, refs) {
  ...
  const rootPromptNames = (await readTxtNames(srcDir)).filter((name) => name !== "model.txt")
