...
import { loadModelPairs, loadPrefixPairs, loadTools, rewriteByPrefix } from "../src/overlay-helpers.ts"
...
test("loadPrefixPairs rejects empty directories", async () => {
  ...
  await expect(loadPrefixPairs(sourceDir, overrideDir)).rejects.toThrow(/No \.ref files/)
})
...
test("loadPrefixPairs reads exact prefix pairs", async () => {
  ...
  expect(pairs).toEqual([{ builtin: "builtin alpha", override: "override alpha" }])
})

test("rewriteByPrefix rewrites matching prefixes", () => {
  expect(rewriteByPrefix("hello world", [{ builtin: "hello", override: "hi" }])).toBe("hi world")
})
